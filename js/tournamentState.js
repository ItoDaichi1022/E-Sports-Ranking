...
//
// 公開前の大会には、状態そのものを出さない ── タグが付くのは「公開する」を
// 押した瞬間からで、そこで初めて「募集中」になる。
//
// 一度は「公開後の姿」をタグに出してみたが、これは間違いだった。公開前の
// プレビューに「募集中」と出るので、まだ誰も入れないのに募集が始まったように
// 見える。状態の言葉は、その状態に本当になってから出す。
...
export function entryState(tournament, now = Date.now()) {
  ...
  // 公開前。状態の言葉は出さない（label は空。上の STATUS_LABELS の説明を参照）。
  // 呼ぶ側は label が空ならタグごと出さないこと。
  if (status === 'draft') {
    return {
      ...base, key: 'draft', label: '', tone: 'draft',

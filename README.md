...
- 対戦カードから**配信用スコアボード**を開ける（運営だけ）。大会ロゴを中心に、
  左右へ選手アイコン・名前・ゲームカウントが並ぶ1枚の絵で、そのURLを
  **OBSのブラウザソース**に貼るとスコアボードだけが映像に乗る（背景は透過）。
  試合中のカウントは配信卓の人が画面上で動かす（操作欄はマウスを動かしたときだけ
  出るので、映像には映らない）
- 対戦表の**自分のいるところをタップすると、その対戦相手とチャット**できる（運営はどの対戦にも入れる）
- もめたときはチャットから**運営に報告**でき、運営の画面ではその大会と対戦に印が出る
- 選手ページからその人を**通報**できる。3人ぶん集まるとサイトの持ち主の画面に
  「BAN対象」として並ぶが、**自動では止めない**（読んで判断するのは人）
- 利用を停止された人は、エントリー・大会作成・チャット・プロフィールの編集ができなくなり、
  選手の検索から消える（**過去の対戦表と戦績は残す** ── 消すと他の選手の記録まで壊れる）
...
js/
  ...
  scoreboard.js        配信用スコアボード。OBSのブラウザソースに貼るURLで、
                       ヘッダーも背景も無い1枚の絵だけを返す（css/scoreboard.css）
...
supabase/
  ...
  migration-002.sql    構築済みプロジェクトへの差分適用（〜023まで番号順に当てる）
  SETUP.md             セットアップ手順
scripts/

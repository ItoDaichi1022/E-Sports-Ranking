...
2. [`supabase/migration-002.sql`](migration-002.sql) の中身を貼り付けて「Run」

当たる内容は次の2つです。

- **定員チェックの作り直し** — 定員ちょうどまで埋まった大会が締め切れなくなる不具合と、
  まとめて登録したときに定員が素通りする不具合の修正
- **アイコン画像** — `players.avatar_url` 列と、画像を置く `avatars` バケット

バケットは「誰でも閲覧可・書き込みは自分のフォルダのみ」で作られます。画像は
